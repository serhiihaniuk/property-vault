...
| `T37` | Redesign dashboard to the finance-first MVP | `claimed` | `T30`, `T34`, `T36` | dashboard views and widgets only | `gpt-5.4 / high` | `redesign-c` | `strong` | dashboard presents snapshot-first current state, balance/ledger, category drill-down cards, trend, anomalies, and recent evidence in the new system |
